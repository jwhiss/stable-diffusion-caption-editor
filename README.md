...
Try out the original MVP from kznrluk [here](https://dagger.anyfrog.net). This forked version contains additional improvements such as a more fully-featured editor mode and vulnerability fixes.
...
By clicking in View Layout, only images that **have** the selected tag(s) will be displayed.

By clicking in Edit Layout, you can add or remove tags in bulk. The "Add Tags" section displays tags that are present in the dataset but not applied to at least one of the selected images, and clicking the tag will add it to all selected images. The "Remove Tags" section displays all tags that are currently applied to at least one of the selected images, and clicking the tag will remove the tag from all selected images.
...
Please use GitHub's Issue tracker for feature requests and bug reports. Pull requests are also welcome.
